// profile.js (الكود الكامل بعد التعديل)
...
const ProfileScreen = () => {
  ...
  const loadData = useCallback(async () => { try { const userJson = await AsyncStorage.getItem('userProfile'); if (userJson) setUserData(JSON.parse(userJson)); const themeValue = await AsyncStorage.getItem('isDarkMode'); setIsDarkMode(themeValue === 'true'); const langValue = await AsyncStorage.getItem('appLanguage'); if (langValue) { setLanguage(langValue); I18nManager.forceRTL(langValue === 'ar'); } } catch (e) { console.error("Failed to load data.", e); } }, []);
  ...
  const handleLogout = async () => { try { await AsyncStorage.removeItem('userProfile'); navigation.reset({ index: 0, routes: [{ name: 'Login' }] }); } catch (e) { Alert.alert(t('logoutErrorTitle'), t('logoutErrorMessage')); } };
...
const styles = {
  ...
  profileImage: (theme) => ({ width: 100, height: 100, borderRadius: 50, borderWidth: 4, borderColor: theme.borderColor, backgroundColor: '#E0E0E0' }),
