// FoodLogScreen.js (النسخة النهائية مع الإعلان البيني العادي والإعلان بمكافأة)
...
// ---===[ تعديل (1/6): استيراد مكونات كلا الإعلانين ]===---
import { InterstitialAd, RewardedInterstitialAd, AdEventType, TestIds } from 'react-native-google-mobile-ads';
// ---===[ نهاية التعديل (1/6) ]===---
...
// ---===[ تعديل (2/6): تعريف معرّفات الوحدتين الإعلانيتين ]===---
// الإعلان البيني العادي
const interstitialAdUnitId = __DEV__ ? TestIds.INTERSTITIAL : 'ca-app-pub-8833281523608204/5182428859';
// الإعلان بمكافأة الجديد (من صورتك)
const rewardedInterstitialAdUnitId = __DEV__ ? TestIds.REWARDED_INTERSTITIAL : 'ca-app-pub-8833281523608204/2734576529';
// ---===[ نهاية التعديل (2/6) ]===---
...
const FoodLogScreen = ({ language, darkMode }) => {
    // ---===[ تعديل (3/6): إضافة حالات ومراجع للإعلانين ]===---
    const [interstitial, setInterstitial] = useState(null);
    const [isInterstitialLoaded, setIsInterstitialLoaded] = useState(false);
    const lastInterstitialShownTime = useRef(0);
    ...
    // ---===[ تعديل (4/6): تهيئة كلا الإعلانين ]===---
    useEffect(() => {
        const interstitialAd = InterstitialAd.createForAdRequest(interstitialAdUnitId, { requestNonPersonalizedAdsOnly: true });
        const unsubscribeInterstitialLoaded = interstitialAd.addAdEventListener(AdEventType.LOADED, () => setIsInterstitialLoaded(true));
        ...
        const rewardedAd = RewardedInterstitialAd.createForAdRequest(rewardedInterstitialAdUnitId, { requestNonPersonalizedAdsOnly: true });
        const unsubscribeRewardedLoaded = rewardedAd.addAdEventListener(AdEventType.LOADED, () => setIsRewardedAdLoaded(true));
        ...
        const unsubscribeEarned = rewardedAd.addAdEventListener(
            ...
            reward => {
                setDailyTargetCalories(prevTarget => prevTarget + 50);
                Alert.alert(translation.rewardSuccess, translation.rewardMessage);
            }
        );
        rewardedAd.load();
        setRewardedInterstitial(rewardedAd);
    ...
    const showInterstitialAd = useCallback(() => {
        ...
        if (now - lastInterstitialShownTime.current < INTERSTITIAL_COOLDOWN_SECONDS) {
            console.log('Interstitial Ad cooldown active.'); return;
        }
        if (isInterstitialLoaded && interstitial) {
            interstitial.show();
            lastInterstitialShownTime.current = now;
        } else {
    ...
    const showRewardedAd = useCallback(() => {
        if (isRewardedAdLoaded && rewardedInterstitial) {
            rewardedInterstitial.show();
        } else {
            Alert.alert("Sorry", "Ad is not ready yet.");
        }
    }, [isRewardedAdLoaded, rewardedInterstitial]);
    // ---===[ نهاية التعديل (4/6) ]===---
    ...
    }, [mealName, mealCalories, mealProtein, mealFat, mealCarbs, selectedMealCategory, translation, handleCloseAddMealModal, isTodaySelected, showInterstitialAd]);

    // ---===[ تعديل (5/6): دالة لفتح نافذة إضافة الوجبة مع التحقق ]===---
    const handleOpenAddMeal = (categoryKey = selectedMealCategory) => {
        ...
        if (remainingCalories <= 0) {
            Alert.alert( translation.earnExtraCaloriesTitle, translation.earnExtraCaloriesMessage,
                [ { text: translation.noThanks, style: "cancel" }, { text: translation.watchAd, onPress: showRewardedAd } ]
            );
        } else {
    ...
    // ---===[ تعديل (6/6): تعديل أزرار `onPress` في كود العرض ]===---
    return (
