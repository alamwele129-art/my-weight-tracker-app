...
const translations = {
  en: {
    ...
    manageSubscription: 'Manage Subscription', // --- ADDED ---
    premiumMember: 'Premium Member',         // --- ADDED ---
  },
  ar: {
    ...
    manageSubscription: 'إدارة الاشتراك',      // --- ADDED ---
    premiumMember: 'عضو مميز',               // --- ADDED ---
  },
};
...
// --- AsyncStorage Keys (MODIFIED) ---
const USER_PROFILE_DATA_KEY = '@Profile:userProfileData';
const LOGGED_IN_EMAIL_KEY = 'loggedInUserEmail';
const USER_SUBSCRIPTION_STATUS_KEY = '@App:userSubscriptionStatus'; // --- ADDED ---

// --- Dynamic Style Generation Function (MODIFIED) ---
const getStyles = (themeMode) => {
  ...
  return StyleSheet.create({
    ...
    userNameContainer: { flexDirection: I18nManager.isRTL ? 'row-reverse' : 'row', alignItems: 'center', justifyContent: 'center', marginBottom: 4 }, // --- ADDED ---
    userName: { fontSize: 20, fontWeight: 'bold', color: theme.text, textAlign: 'center' },
    premiumBadge: { width: 20, height: 20, marginLeft: I18nManager.isRTL ? 0 : 8, marginRight: I18nManager.isRTL ? 8 : 0 }, // --- ADDED ---
...
const ProfileScreen = ({
  ...
  const loadProfileData = useCallback(async () => {
    try {
      ...
      if (userProfileDataString) {
        ...
        loadedImageUrl = profileData.profileImageUrl;
      }
      const loggedInUserEmail = await AsyncStorage.getItem(LOGGED_IN_EMAIL_KEY);
      
      const subscriptionStatus = await AsyncStorage.getItem(USER_SUBSCRIPTION_STATUS_KEY);
      setIsPremium(subscriptionStatus === 'true');
      ...
      const errorT = (key) => translations['en']?.[key] || key;
        ...
        { text: t('cancel'), style: 'cancel' },
        {
          ...
          onPress: async () => {
            try {
              ...
              await AsyncStorage.multiRemove([LOGGED_IN_EMAIL_KEY, USER_PROFILE_DATA_KEY, USER_SUBSCRIPTION_STATUS_KEY]);
  ...
  const profileImageSource = (profileImageUri === DEFAULT_PROFILE_IMAGE_URI || !profileImageUri) ? DEFAULT_PROFILE_ASSET : { uri: profileImageUri };
      ...
      <ScrollView style={styles.scrollContainer} contentContainerStyle={styles.scrollContentContainer}>
        <View style={styles.card}>
           ...
           <View style={styles.profilePicContainer}>
             <Image source={profileImageSource} style={styles.profilePic} key={imageKey} resizeMode="cover" onError={() => setProfileImageUri(DEFAULT_PROFILE_IMAGE_URI)} />
           </View>
           ...
           <View style={styles.menuContainer}>
             {isPremium ? (
                <View style={styles.menuItem}>
                    ...
                    <TouchableOpacity onPress={() => alert(t('manageSubscription'))} activeOpacity={0.6}>
