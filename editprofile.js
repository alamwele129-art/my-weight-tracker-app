...
// --- الترجمات والثيمات والأدوات المساعدة (تبقى كما هي) ---
const translations = {
  en: { editProfile: 'EDIT PROFILE', publicInfo: 'PUBLIC INFORMATION', firstName: 'First name', lastName: 'Last name', mail: 'Mail', physicalMetrics: 'PHYSICAL METRICS', gender: 'Gender', male: 'Male', female: 'Female', dob: 'Date of Birth', height: 'Height (cm)', currentWeight: 'Current Weight (kg)', goals: 'GOALS', mainGoal: 'Main Goal', lose: 'Lose', maintain: 'Maintain', gain: 'Gain', targetWeight: 'Target Weight (kg)', activityLevel: 'Activity Level', sedentary: 'Sedentary', light: 'Light', active: 'Active', veryActive: 'Very Active', profilePic: 'Profile Picture', chooseNewPic: 'Choose your new picture', takePhoto: 'Take Photo', chooseFromGallery: 'Choose from Gallery', cancel: 'Cancel', success: 'Success', saveSuccess: 'Changes saved successfully!', error: 'Error', saveError: 'An error occurred while saving data.', },
  ar: { editProfile: 'تعديل الملف الشخصي', publicInfo: 'المعلومات العامة', firstName: 'الاسم الأول', lastName: 'الاسم الأخير', mail: 'البريد الإلكتروني', physicalMetrics: 'المقاييس البدنية', gender: 'الجنس', male: 'ذكر', female: 'أنثى', dob: 'تاريخ الميلاد', height: 'الطول (سم)', currentWeight: 'الوزن الحالي (كغ)', goals: 'الأهداف', mainGoal: 'الهدف الرئيسي', lose: 'خسارة وزن', maintain: 'الحفاظ على الوزن', gain: 'زيادة وزن', targetWeight: 'الوزن المستهدف (كغ)', activityLevel: 'مستوى النشاط', sedentary: 'خامل', light: 'خفيف', active: 'نشيط', veryActive: 'نشيط جداً', profilePic: 'صورة الملف الشخصي', chooseNewPic: 'اختر صورتك الجديدة', takePhoto: 'التقاط صورة', chooseFromGallery: 'اختيار من المعرض', cancel: 'إلغاء', success: 'نجاح', saveSuccess: 'تم حفظ التغييرات بنجاح!', error: 'خطأ', saveError: 'حدث خطأ أثناء حفظ البيانات.', },
};
...
const EditProfileScreen = () => {
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      // [تعديل] الانتقال مباشرة بعد الحفظ بنجاح
      navigation.goBack();
    } catch (error) {
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: keyboardPadding }} keyboardShouldPersistTaps="handled">
        <View style={styles.header}>
          <TouchableOpacity onPress={() => navigation.goBack()}>
            <Ionicons name={isRTL ? "chevron-forward" : "chevron-back"} size={28} color={theme.icon} />
          </TouchableOpacity>
        ...
        <View style={styles.profileSection}><View style={styles.profileImageContainer}><Image source={profileImage ? { uri: profileImage } : require('./assets/profile.png')} style={styles.profileImage}/><TouchableOpacity style={styles.cameraButton} onPress={handleImagePicker}><Ionicons name="camera" size={18} color={theme.textDark} /></TouchableOpacity></View></View>
        <View style={styles.formSection}><Text style={styles.sectionTitle}>{t('publicInfo')}</Text><InfoInput label={t('firstName')} value={firstName} onChangeText={setFirstName} theme={theme} isRTL={isRTL}/><InfoInput label={t('lastName')} value={lastName} onChangeText={setLastName} theme={theme} isRTL={isRTL}/><View style={[styles.inputContainer, styles.disabledInputContainer]}><View style={{flex: 1}}><Text style={styles.inputLabel}>{t('mail')}</Text><TextInput style={[styles.textInput, styles.disabledTextInput]} value={email} editable={false} /></View><Ionicons name="lock-closed-outline" size={22} color={theme.textGray} /></View></View>
        <View style={styles.formSection}><Text style={styles.sectionTitle}>{t('physicalMetrics')}</Text><OptionSelector label={t('gender')} options={[{ label: t('male'), value: 'male' }, { label: t('female'), value: 'female' }]} selectedValue={gender} onSelect={setGender} theme={theme} isRTL={isRTL} /><TouchableOpacity style={styles.inputContainer} onPress={() => setShowDatePicker(true)}><View style={{flex: 1}}><Text style={styles.inputLabel}>{t('dob')}</Text><Text style={styles.textInput}>{birthDate.toLocaleDateString(activeLanguage === 'ar' ? 'ar-EG' : 'en-GB')}</Text></View><Ionicons name="calendar-outline" size={22} color={theme.textGray} /></TouchableOpacity>{showDatePicker && <DateTimePicker value={birthDate} mode="date" display="spinner" onChange={onDateChange} locale={activeLanguage} />}<InfoInput label={t('height')} value={height} onChangeText={setHeight} keyboardType="numeric" theme={theme} isRTL={isRTL}/><InfoInput label={t('currentWeight')} value={weight} onChangeText={setWeight} keyboardType="numeric" theme={theme} isRTL={isRTL}/></View>
        <View style={styles.formSection}><Text style={styles.sectionTitle}>{t('goals')}</Text><OptionSelector label={t('mainGoal')} options={[{ label: t('lose'), value: 'lose' }, { label: t('maintain'), value: 'maintain' }, { label: t('gain'), value: 'gain' }]} selectedValue={goal} onSelect={setGoal} theme={theme} isRTL={isRTL} />{goal !== 'maintain' && <InfoInput label={t('targetWeight')} value={targetWeight} onChangeText={setTargetWeight} keyboardType="numeric" theme={theme} isRTL={isRTL}/>}<OptionSelector label={t('activityLevel')} options={[{ label: t('sedentary'), value: 'sedentary' }, { label: t('light'), value: 'light' }, { label: t('active'), value: 'active' }, { label: t('very_active'), value: 'very_active' }]} selectedValue={activityLevel} onSelect={setActivityLevel} theme={theme} isRTL={isRTL}/></View>
      </ScrollView>
    </SafeAreaView>
  );
};

// --- دالة الستايلات (تبقى كما هي) ---
const getStyles = (theme, isRTL) => StyleSheet.create({
  container: { flex: 1, backgroundColor: theme.background }, header: { flexDirection: isRTL ? 'row-reverse' : 'row', justifyContent: 'space-between', alignItems: 'center', paddingHorizontal: 20, paddingVertical: 15 }, headerTitle: { fontSize: 18, fontWeight: 'bold', color: theme.textDark }, profileSection: { alignItems: 'center', marginVertical: 20 }, profileImageContainer: { position: 'relative' }, profileImage: { width: 100, height: 100, borderRadius: 50, backgroundColor: theme.surface, borderWidth: 1, borderColor: theme.border }, cameraButton: { position: 'absolute', bottom: 0, right: isRTL ? undefined : 0, left: isRTL ? 0 : undefined, backgroundColor: theme.surface, width: 32, height: 32, borderRadius: 16, justifyContent: 'center', alignItems: 'center', borderWidth: 1, borderColor: theme.border, elevation: 3 }, formSection: { paddingHorizontal: 20, marginBottom: 10 }, sectionTitle: { fontSize: 13, color: theme.textGray, fontWeight: '600', marginBottom: 15, textTransform: 'uppercase', textAlign: isRTL ? 'right' : 'left' }, inputContainer: { backgroundColor: theme.surface, borderRadius: 12, paddingVertical: 12, paddingHorizontal: 16, marginBottom: 15, flexDirection: isRTL ? 'row-reverse' : 'row', justifyContent: 'space-between', alignItems: 'center', borderWidth: 1, borderColor: theme.border }, inputLabel: { fontSize: 12, color: theme.textGray, marginBottom: 4, textAlign: isRTL ? 'right' : 'left' }, textInput: { fontSize: 16, fontWeight: '600', color: theme.textDark, padding: 0, textAlign: isRTL ? 'right' : 'left' }, disabledInputContainer: { backgroundColor: theme.disabledBackground }, disabledTextInput: { color: theme.textGray }, optionContainer: { marginBottom: 15 }, optionsWrapper: { flexDirection: isRTL ? 'row-reverse' : 'row', justifyContent: 'space-between' }, optionButton: { flex: 1, paddingVertical: 12, borderRadius: 8, borderWidth: 1, borderColor: theme.border, alignItems: 'center', marginHorizontal: 4, backgroundColor: theme.surface }, optionButtonSelected: { backgroundColor: theme.primary, borderColor: theme.primary }, optionText: { color: theme.textDark, fontWeight: '600' }, optionTextSelected: { color: '#FFFFFF' },
});
