// SignUpScreen.js (With Facebook instead of Apple - FULL CODE)
...
const SignUpScreen = () => {
    ...
    // <<< 3. إضافة دالة تسجيل الدخول عبر فيسبوك >>>
    const handleFacebookSignIn = async () => {
        if (isLoading || isGoogleLoading || isFacebookLoading) return;
        ...
        try {
            ...
            if (error) {
                ...
                Alert.alert('خطأ في تسجيل الدخول بفيسبوك', error.message);
            } else {
                console.log('Facebook Sign-In successful:', data);
            }
        } catch (error) {
            console.error('Unexpected Facebook Sign-In Error:', error);
            Alert.alert('خطأ', 'حدث خطأ غير متوقع أثناء تسجيل الدخول بفيسبوك.');
        } finally {
      ...
      >
        <View style={styles.content}>
          ...
          <TouchableOpacity
              ...
              disabled={isLoading || isGoogleLoading || isFacebookLoading || !request}
          >
            {isGoogleLoading ? (
          ...
          <View style={styles.inputContainer}>
            ...
            <TextInput
              ...
              editable={!isLoading && !isGoogleLoading && !isFacebookLoading}
              blurOnSubmit={false}
          ...
          <View style={styles.inputContainer}>
            ...
            />
            <TouchableOpacity onPress={() => setShowNewPassword(!showNewPassword)} disabled={isLoading || isGoogleLoading || isFacebookLoading} style={styles.eyeIcon}>
          ...
          <View style={styles.inputContainer}>
            ...
            <TextInput
              ...
              textContentType="newPassword"
            />
            <TouchableOpacity onPress={() => setShowConfirmPassword(!showConfirmPassword)} disabled={isLoading || isGoogleLoading || isFacebookLoading} style={styles.eyeIcon}>
          ...
          <TouchableOpacity
             style={[
                 styles.mainButton,
                 (isLoading || isGoogleLoading || isFacebookLoading || !agreeTerms || !passwordValid.length || !passwordValid.number || !passwordValid.case) && styles.mainButtonDisabled
             ]}
             onPress={handleSignUp}
             disabled={isLoading || isGoogleLoading || isFacebookLoading || !agreeTerms || !passwordValid.length || !passwordValid.number || !passwordValid.case}
           >
             {isLoading ? (
