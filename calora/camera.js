...
// =====================================================================================
// --- 1. الثوابت، الثيمات، الترجمات، والمكونات المساعدة ---
// =====================================================================================

// 🛑🛑🛑 هام: استبدل هذا بمفتاح Google Cloud Vision API الخاص بك 🛑🛑🛑
// الأفضل: قم بتخزينه في متغيرات البيئة (environment variables) ولا تضعه هنا مباشرة
...
const lightTheme = {
    background: '#FCFCFC', text: '#212121', secondaryText: '#757575', primary: '#4CAF50',
    danger: '#F44336', modalBackdrop: 'rgba(0,0,0,0.5)', modalSurface: 'white',
    macrosBackground: '#F1F8E9', cancelButtonBackground: '#E8F5E9', cancelButtonText: '#4CAF50',
    permissionIcon: '#BDBDBD', captureButton: 'white', captureBorder: 'rgba(255,255,255,0.5)',
    cameraBackground: '#000000', textOnDark: '#FFFFFF', modeSelectorBackground: 'rgba(40,40,40,0.8)',
    modeSelectorActive: 'rgba(255,255,255,0.9)'
};

const darkTheme = {
    background: '#121212', text: '#FFFFFF', secondaryText: '#A5A5A5', primary: '#4CAF50',
    danger: '#EF5350', modalBackdrop: 'rgba(0,0,0,0.7)', modalSurface: '#1E1E1E',
    macrosBackground: '#2C3B2A', cancelButtonBackground: '#2C3B2A', cancelButtonText: '#4CAF50',
    permissionIcon: '#757575', captureButton: '#333333', captureBorder: 'rgba(255,255,255,0.3)',
    cameraBackground: '#000000', textOnDark: '#FFFFFF', modeSelectorBackground: 'rgba(50,50,50,0.9)',
    modeSelectorActive: 'rgba(255,255,255,0.2)'
};

const translations = {
    ar: {
        analyzingPlate: 'جاري تحليل طبقك...', analysisFailed: 'فشل تحليل الصورة.', permissionDenied: 'لا يمكن الوصول للكاميرا', grantPermission: 'منح الإذن', plateResults: 'نتائج طبقك',
        dailyGoal: 'هدف اليوم', plateCalories: 'سعرات الطبق', remaining: 'متبقي', afterAddingMeal: 'بعد إضافة الوجبة', carbs: 'كربوهيدرات', protein: 'بروتين',
        fat: 'دهون', fiber: 'ألياف', sugar: 'سكر', sodium: 'صوديوم', cancel: 'إلغاء', addToDiary: 'إضافة لليوميات', mealAddedSuccess: 'تم إضافة الوجبة بنجاح!',
        mealAddedError: 'حدث خطأ أثناء حفظ الوجبة.', chooseMeal: 'اختر الوجبة', breakfast: 'الفطور', lunch: 'الغداء', dinner: 'العشاء', snacks: 'وجبات خفيفة',
        scanFood: 'مسح الطعام', barcode: 'باركود', scanning: 'جاري البحث...', productNotFound: 'لم يتم العثور على المنتج', editQuantity: 'تعديل الكمية',
        newQuantity: 'الكمية الجديدة (جرام)', confirm: 'تأكيد', invalidNumber: 'الرجاء إدخال رقم صحيح.', plateTotal: 'إجمالي الطبق', foodIdentified: 'تم التعرف على الأكلة',
        confirmAdd: 'هل تريد إضافة "{foodName}" ({calories} سعر حراري)؟', tryAgain: 'حاول مرة أخرى', notInLocalDB: 'تعرفنا على الأكلة كـ "{foodName}" ولكنها غير موجودة في قاعدة البيانات المحلية. جاري البحث في قاعدة البيانات العالمية...',
        // ✅ --- نصوص جديدة لميزة OCR --- ✅
        scanLabel: 'ملصق', analyzingLabel: 'جاري قراءة الملصق...', labelReadSuccess: 'تمت قراءة الملصق بنجاح!', enterProductName: 'أدخل اسم المنتج', productName: 'اسم المنتج', pleaseEnterName: 'الرجاء إدخال اسم للمنتج.', noNutritionDataFound: 'لم نتمكن من العثور على بيانات غذائية في الصورة.'
    },
    // ... no changes to english translations needed for this request
};
...
const ProductNameModal = ({ visible, onClose, onConfirm, theme, t, isRTL }) => {
    ...
    return (
        <Modal visible={visible} transparent={true} animationType="fade">
            <View style={styles.mealSelectionBackdrop}>
                <View style={[styles.mealSelectionContainer, { alignItems: 'stretch' }]}>
                    <Text style={styles.mealSelectionTitle}>{t('enterProductName')}</Text>
                    <TextInput
                    ...
                    <View style={styles.modalActions}>
                        ...
                        <TouchableOpacity style={[styles.actionButton, styles.addButton]} onPress={handleConfirm}>
...
const CameraScreen = () => {
    ...
    // ✅ --- حالة جديدة للتحكم بمربع حوار إدخال الاسم --- ✅
    const [isNameModalVisible, setNameModalVisible] = useState(false);
    const [parsedOcrData, setParsedOcrData] = useState(null);


    const t = useCallback((key, params = {}) => {
        let str = (translations[language] || translations['en'])?.[key] || key;
        Object.keys(params).forEach(p => { str = str.replace(`{${p}}`, params[p]); });
        return str;
    }, [language]);
    const styles = getStyles(theme, isRTL);

    const requestPermission = async () => { const { status } = await Camera.requestCameraPermissionsAsync(); setHasPermission(status === 'granted'); };
    ...
    const takePicture = async () => {
        if (cameraRef.current) {
            ...
            } else if (scanMode === 'label') { // ✅ --- التعامل مع وضع قراءة الملصق --- ✅
                analyzeLabelPhoto(photo.base64);
            }
        }
    };
    
    // ... (analyzePhoto and other functions remain the same)
    const analyzePhoto = async (imageBase64) => {
        ...
        try {
            const clarifaiResponse = await fetch("https://api.clarifai.com/v2/models/food-item-recognition/outputs", {
                method: 'POST',
                headers: { 'Accept': 'application/json', 'Authorization': 'Key ' + CLARIFAI_PAT },
                body: JSON.stringify({ "user_app_id": { "user_id": CLARIFAI_USER_ID, "app_id": CLARIFAI_APP_ID }, "inputs": [{ "data": { "image": { "base64": imageBase64 } } }] })
            });
            ...
            if (localResults.length > 0) {
                const matchedFood = { ...localResults[0], quantity: 100 };
                setAnalysisResult([matchedFood]);
                setBaseAnalysisResult([matchedFood]);
            } else {
                Alert.alert(t('foodIdentified'), t('notInLocalDB', {foodName: foodNameFromClarifai}));
                const usdaResult = await getNutritionDataFromUSDA(foodNameFromClarifai, USDA_API_KEY);
                if (usdaResult) { setAnalysisResult([usdaResult]); setBaseAnalysisResult([usdaResult]); } 
                else { throw new Error('Food not found in any database.'); }
            }
        } catch (error) {
            console.error("Analysis failed:", error);
            Alert.alert(t('analysisFailed'), error.message, [{ text: t('tryAgain') }]);
        } finally {
            setIsAnalyzing(false);
        }
    };

    // ✅ --- دالة جديدة لتحليل صورة الملصق الغذائي باستخدام Google Vision API --- ✅
    const analyzeLabelPhoto = async (imageBase64) => {
        if (GOOGLE_VISION_API_KEY === 'YOUR_GOOGLE_CLOUD_VISION_API_KEY') {
            Alert.alert("Error", "Please add your Google Cloud Vision API key to the code.");
            return;
        }
        setIsAnalyzing(true);
        try {
            ...
            if (result.responses && result.responses[0].fullTextAnnotation) {
                ...
                const nutritionData = parseNutritionText(detectedText);

                if (Object.values(nutritionData).some(val => val > 0)) {
                    setParsedOcrData(nutritionData);
                    setNameModalVisible(true); // اطلب من المستخدم إدخال الاسم
                } else {
                    Alert.alert(t('analysisFailed'), t('noNutritionDataFound'));
                }
            } else {
            ...
            Alert.alert(t('analysisFailed'), error.message);
        } finally {
            setIsAnalyzing(false);
        }
    };

    // ✅ --- دالة ذكية لاستخراج البيانات من النص باستخدام Regex --- ✅
    const parseNutritionText = (text) => {
        const cleanedText = text.replace(/,/g, '.'); // توحيد الفاصلة العشرية
        const nutrition = { calories: 0, p: 0, c: 0, f: 0, fib: 0, sug: 0, sod: 0 };

        const patterns = {
            calories: /(calories|energy|السعرات الحرارية|طاقة|kcal)\s*(\d+(\.\d+)?)/i,
            p: /(protein|بروتين)\s*(\d+(\.\d+)?)\s*g?/i,
            c: /(carbohydrate|الكربوهيدرات|carbs)\s*(\d+(\.\d+)?)\s*g?/i,
            f: /(fat|total fat|الدهون)\s*(\d+(\.\d+)?)\s*g?/i,
            fib: /(fiber|ألياف)\s*(\d+(\.\d+)?)\s*g?/i,
            sug: /(sugars|سكر)\s*(\d+(\.\d+)?)\s*g?/i,
            sod: /(sodium|صوديوم)\s*(\d+(\.\d+)?)\s*m?g?/i,
        };

        for (const key in patterns) {
            const match = cleanedText.match(patterns[key]);
            if (match && match[2]) {
                nutrition[key] = parseFloat(match[2]);
            }
        }
        
        // تحويل الصوديوم من مليجرام إلى جرام إذا كان الرقم كبيراً جداً (افتراض)
        if (nutrition.sod > 10000) nutrition.sod /= 1000;

        return nutrition;
    };
    
    // ✅ --- دالة لتأكيد اسم المنتج ودمجه مع بيانات OCR --- ✅
    const handleConfirmProductName = (name) => {
        const finalItem = {
            id: `ocr-${Date.now()}`,
            name: name,
            quantity: 100, // نفترض 100 جرام كقاعدة
            ...parsedOcrData
        };
        setAnalysisResult([finalItem]);
        setBaseAnalysisResult([finalItem]);
    ...
    if (hasPermission === null) return <View style={styles.permissionContainer}><ActivityIndicator size="large" color={theme.primary} /></View>;
    ...
    const getLoadingText = () => {
        if (scanMode === 'food') return t('analyzingPlate');
        if (scanMode === 'barcode') return t('scanning');
        if (scanMode === 'label') return t('analyzingLabel');
        return '';
    };

    return (
        <View style={styles.container}>
            <Camera ref={cameraRef} style={StyleSheet.absoluteFill} type={CameraType.back} onBarCodeScanned={scanMode === 'barcode' && !isAnalyzing ? handleBarCodeScanned : undefined}/>
            ...
            {/* ✅ --- إطار توجيهي جديد لوضع قراءة الملصق --- ✅ */}
            ...
            <Modal animationType="slide" transparent={true} visible={!!analysisResult}>
                <View style={styles.modalBackdrop}>
                    <View style={styles.modalContainer}>
                        <FlatList
                            ...
                            ListHeaderComponent={ <> <Text style={styles.modalTitle}>{t('plateResults')}</Text> {analysisResult && analysisResult.length === 1 && analysisResult[0].image ? (<Image source={{ uri: analysisResult[0].image }} style={styles.productImage} />) : null} </> }
                            ...
                            ListFooterComponent={ <View>
                                {analysisResult && analysisResult.length > 1 && (<View style={styles.plateTotalContainer}><Text style={styles.plateTotalLabel}>{t('plateTotal')}</Text><Text style={styles.plateTotalValue}>{Math.round(plateTotals.calories)} Cal</Text></View>)}
...
// ... (باقي كود الأنماط getStyles كما هو ولكن مع إضافة أنماط جديدة)
const getStyles = (theme, isRTL) => StyleSheet.create({
    ...
    bottomContainer: { position: 'absolute', bottom: 0, left: 0, right: 0, alignItems: 'center', paddingBottom: 20 },
    ...
    modeButton: { flexDirection: 'row', alignItems: 'center', paddingVertical: 8, paddingHorizontal: 15, borderRadius: 25, }, // Adjusted padding
    modeButtonActive: { backgroundColor: theme.modeSelectorActive, },
    modeButtonText: { marginLeft: 8, color: theme.secondaryText, fontWeight: '600', fontSize: 14, }, // Adjusted font size
