...
import { searchEgyptianFoodsWithImages, supabase } from './supabaseclient'; // ✅✅ تم استيراد supabase هنا ✅✅
...
// =========================================================================
// --- أنظمة الثيم واللغة (لا يوجد تغيير هنا) ---
// =========================================================================
...
const lightTheme = {
    ...
    disabled: '#BDBDBD',
    carbs: '#007BFF', protein: '#FF7043', fat: '#FFC107',
    fiber: '#4CAF50', sugar: '#9C27B0', sodium: '#2196F3',
    overLimit: '#D32F2F',
    tabBarBackground: '#FFFFFF',
    tabBarIndicator: '#4CAF50',
    tabBarIcon: '#222327',
    white: '#FFFFFF',
    readOnlyBanner: '#FFA000',
    indicatorDot: '#1B5E20',
...
const darkTheme = {
    ...
    disabled: '#424242',
    carbs: '#42A5F5', protein: '#FF8A65', fat: '#FFCA28',
    fiber: '#81C784', sugar: '#BA68C8', sodium: '#64B5F6',
    overLimit: '#EF9A9A',
    tabBarBackground: '#1E1E1E',
    tabBarIndicator: '#81C784',
    tabBarIcon: '#E0E0E0',
    white: '#FFFFFF',
    readOnlyBanner: '#D48604',
    indicatorDot: '#A5D6A7',
...
const translations = {
    ar: {
        // Diary Screen
        remainingCalories: 'سعر حراري متبقي',
        readOnlyBanner: 'أنت تعرض يوماً سابقاً. السجل للقراءة فقط.',
        mealSectionsTitle: 'أقسام الوجبات',
        mealSectionsDesc: 'هذا هو السجل التفصيلي لليوم.',
        breakfast: 'الفطور',
        lunch: 'الغداء',
        dinner: 'العشاء',
        snacks: 'وجبات خفيفة',
        add_to_meal: '+ أضف إلى {meal}',
        // Nutrients
        protein: 'بروتين', carbs: 'كربوهيدرات', fat: 'دهون',
        fiber: 'ألياف', sugar: 'سكر', sodium: 'صوديوم',
        g_unit: 'جم', mg_unit: 'مجم', kcal_unit: 'kcal',
        // Small Cards
        weight: 'الوزن', water: 'الماء', workouts: 'التمارين', steps: 'الخطوات',
        not_logged: 'لم يسجل', unsupported: 'غير مدعوم',
        kg_unit: 'كجم', burned_cal: 'سعر حراري', goal: 'الهدف: ',
        // Food Log
        dailyLogTitle: 'سجل وجبات اليوم',
        // Add Food Modal
        add_to: 'إضافة إلى',
        search_placeholder: 'ابحث عن كشري، ملوخية، تفاح...',
        no_results: 'لا توجد نتائج بحث.',
        local_food: 'أكلة محلية 🇪🇬',
        // Alerts
        error: 'خطأ',
        search_error_msg: 'الرجاء إدخال اسم طعام للبحث.',
        fetch_error_msg: 'حدث خطأ أثناء جلب تفاصيل الطعام.',
        save_error_msg: 'حدث خطأ أثناء حفظ البيانات.',
        // Navigators & Tabs
        diaryTab: 'يومياتي',
        reportsTab: 'تقارير',
        cameraTab: 'كاميرا',
        profileTab: 'حسابي',
        weightTrackerTitle: 'تتبع الوزن',
        waterTrackerTitle: 'تتبع الماء',
        workoutLogTitle: 'سجل التمارين',
        stepsReportTitle: 'تقرير الخطوات',
        foodLogDetailTitle: 'تفاصيل سجل الوجبات',
        // Date Navigator
        weekdays: ['ح', 'ن', 'ث', 'ر', 'خ', 'ج', 'س'],
        //Macros
        p_macro: 'ب: ', c_macro: 'ك: ', f_macro: 'د: ',
        fib_macro: 'أ: ', sug_macro: 'س: ', sod_macro: 'ص: ',
    },
    en: {
        // Diary Screen
        remainingCalories: 'Calories Remaining',
        readOnlyBanner: "You are viewing a past day. The log is read-only.",
        mealSectionsTitle: 'Meal Sections',
        mealSectionsDesc: 'This is the detailed log for the day.',
        breakfast: 'Breakfast',
        lunch: 'Lunch',
        dinner: 'Dinner',
        snacks: 'Snacks',
        ...
        g_unit: 'g', mg_unit: 'mg', kcal_unit: 'kcal',
        // Small Cards
        weight: 'Weight', water: 'Water', workouts: 'Workouts', steps: 'Steps',
        not_logged: 'Not logged', unsupported: 'Unsupported',
        kg_unit: 'kg', burned_cal: 'calories', goal: 'Goal: ',
        // Food Log
        dailyLogTitle: "Today's Food Log",
        // Add Food Modal
        add_to: 'Add to',
        search_placeholder: 'Search for koshari, molokhia, apple...',
        no_results: 'No search results found.',
        local_food: 'Local Food 🇪🇬',
        // Alerts
        error: 'Error',
        search_error_msg: 'Please enter a food name to search.',
        fetch_error_msg: 'An error occurred while fetching food details.',
        save_error_msg: 'An error occurred while saving data.',
        // Navigators & Tabs
        diaryTab: 'Diary',
        reportsTab: 'Reports',
        cameraTab: 'Camera',
        ...
        stepsReportTitle: 'Steps Report',
        foodLogDetailTitle: 'Food Log Details',
        // Date Navigator
        weekdays: ['S', 'M', 'T', 'W', 'T', 'F', 'S'],
         //Macros
        p_macro: 'P: ', c_macro: 'C: ', f_macro: 'F: ',
        fib_macro: 'Fib: ', sug_macro: 'Sug: ', sod_macro: 'Sod: ',
    }
};
...
// --- تمت إضافة هذه الدالة لطلب الأذونات ---
...
const DateNavigator = ({ selectedDate, onDateSelect, referenceToday, theme, t, isRTL, language }) => { 
    ...
    for (let i = 0; i < 7; i++) { const date = new Date(startDate); date.setDate(startDate.getDate() + i); dates.push(date); } 
    ...
    const prevButtonHandler = isRTL ? handlePrevWeek : handleNextWeek;
    const nextButtonHandler = isRTL ? handleNextWeek : handlePrevWeek;
        ...
        <View style={styles.dateNavContainer(theme)}>
            <View style={styles.dateNavHeader(isRTL)}>
                <TouchableOpacity 
                    ...
                    disabled={!isRTL && isNextDisabled}
                >
                    <Ionicons 
                        name="chevron-back-outline" 
                        size={24} 
                        color={!isRTL && isNextDisabled ? theme.disabled : theme.primary} 
                    />
                </TouchableOpacity>
...
function DiaryScreen({ navigation, setHasProgress, theme, t, isRTL, language }) { 
    ...
    const loadAllData = useCallback(async () => { try { const profileJson = await AsyncStorage.getItem('userProfile'); if (profileJson != null) { const savedProfile = JSON.parse(profileJson); if (savedProfile.dailyGoal) setDailyGoal(savedProfile.dailyGoal); } const settingsJson = await AsyncStorage.getItem('waterSettings'); if (settingsJson) { const settings = JSON.parse(settingsJson); setWaterGoal(settings.goal || 8); } else { setWaterGoal(8); } const dateKey = formatDateKey(selectedDate); const dayJson = await AsyncStorage.getItem(dateKey); let currentDayData = dayJson != null ? JSON.parse(dayJson) : { ...EMPTY_DAY_DATA }; const weightHistoryJson = await AsyncStorage.getItem('weightHistory'); const weightHistory = weightHistoryJson ? JSON.parse(weightHistoryJson) : []; if (weightHistory.length > 0) { weightHistory.sort((a, b) => new Date(a.date) - new Date(b.date)); const endOfDay = new Date(selectedDate); endOfDay.setHours(23, 59, 59, 999); const weightForDate = weightHistory.filter(entry => new Date(entry.date) <= endOfDay).pop(); currentDayData.displayWeight = weightForDate ? weightForDate.weight : 0; } else { currentDayData.displayWeight = 0; } setDailyData(currentDayData); } catch (e) { console.error("Failed to load data on focus:", e); setDailyData(EMPTY_DAY_DATA); } }, [selectedDate]); 
...
const MagicLineTabBar = ({ state, descriptors, navigation, theme, t }) => { const TAB_COUNT = state.routes.length; const TAB_WIDTH = SCREEN_WIDTH / TAB_COUNT; const translateX = useSharedValue(0); const [profileImage, setProfileImage] = useState(null); useEffect(() => { translateX.value = withTiming(state.index * TAB_WIDTH, { duration: 500 }); }, [state.index, TAB_WIDTH]); useFocusEffect(useCallback(() => { const loadProfileImage = async () => { try { const jsonValue = await AsyncStorage.getItem('userProfile'); if (jsonValue != null) { const data = JSON.parse(jsonValue); setProfileImage(data.profileImage || null); } else { setProfileImage(null); } } catch (e) { console.error("Failed to load profile image for tab bar:", e); } }; loadProfileImage(); }, [])); const indicatorAnimatedStyle = useAnimatedStyle(() => ({ transform: [{ translateX: translateX.value }] })); return (<View style={styles.tabBarContainer(theme)}><View style={styles.animationWrapper}><LeafAnimation trigger={state.index} /></View><Animated.View style={[styles.indicatorContainer, { width: TAB_WIDTH }, indicatorAnimatedStyle]}><View style={[styles.indicator(theme), { backgroundColor: theme.tabBarIndicator }]}><View style={[styles.cutout, styles.cutoutLeft(theme)]} /><View style={[styles.cutout, styles.cutoutRight(theme)]} /></View></Animated.View>{state.routes.map((route, index) => { const { options } = descriptors[route.key]; const isFocused = state.index === index; const onPress = () => { const event = navigation.emit({ type: 'tabPress', target: route.key, canPreventDefault: true }); if (!isFocused && !event.defaultPrevented) { navigation.navigate(route.name); } }; const iconAnimatedStyle = useAnimatedStyle(() => ({ transform: [{ translateY: withTiming(isFocused ? -32 : 0, { duration: 500 }) }] })); const textAnimatedStyle = useAnimatedStyle(() => ({ opacity: withTiming(isFocused ? 1 : 0, { duration: 500 }), transform: [{ translateY: withTiming(isFocused ? 10 : 20, { duration: 500 }) }] })); const isProfileTab = route.name === 'Profile'; return (<TouchableOpacity key={route.key} style={[styles.tabItem, { width: TAB_WIDTH }]} onPress={onPress}><Animated.View style={iconAnimatedStyle}>{isProfileTab ? (<Image source={profileImage ? { uri: profileImage } : require('./assets/profile.png')} style={styles.profileTabIcon} />) : (<Ionicons name={options.tabBarIconName || 'alert-circle-outline'} size={28} color={theme.tabBarIcon} />)}</Animated.View><Animated.Text style={[styles.tabText(theme), textAnimatedStyle]}>{options.tabBarLabel}</Animated.Text></TouchableOpacity>); })}</View>); };
const commonStackOptions = (theme) => ({ headerStyle: { backgroundColor: theme.primary }, headerTintColor: theme.white, headerTitleStyle: { fontWeight: 'bold' }, headerTitleAlign: 'center', cardStyle: { flex: 1 }, });
const DiaryStack = createStackNavigator();
function DiaryStackNavigator({ theme, t, isRTL, language, setHasProgress }) { return ( <DiaryStack.Navigator screenOptions={commonStackOptions(theme)}><DiaryStack.Screen name="DiaryHome" options={{ headerShown: false }}>{props => <DiaryScreen {...props} setHasProgress={setHasProgress} theme={theme} t={t} isRTL={isRTL} language={language} />}</DiaryStack.Screen><DiaryStack.Screen name="Weight" component={WeightScreen} options={{ title: t('weightTrackerTitle') }} /><DiaryStack.Screen name="Water" component={WaterScreen} options={{ title: t('waterTrackerTitle') }} /><DiaryStack.Screen name="WorkoutLog" component={WorkoutLogScreen} options={{ title: t('workoutLogTitle') }} /><DiaryStack.Screen name="Steps" component={StepsScreen} options={{ title: t('stepsReportTitle') }} /><DiaryStack.Screen name="FoodLogDetail" component={FoodLogDetailScreen} options={{ title: t('foodLogDetailTitle') }} /></DiaryStack.Navigator> ); }
const ReportsStack = createStackNavigator();
function ReportsStackNavigator({ theme }) { return ( <ReportsStack.Navigator screenOptions={commonStackOptions(theme)}><ReportsStack.Screen name="ReportsHome" component={ReportsScreen} options={{ headerShown: false }} /></ReportsStack.Navigator> ); }
...
function MainUIScreen() {
  ...
  // --- START: هذا هو الكود الكامل والجديد بدلاً من التعليق ---
  useEffect(() => {
    const setupNotificationsAndBackgroundTasks = async () => {
      try {
        ...
        if (!isTaskRegistered) {
          ...
          console.log("تم تسجيل مهمة التحقق من الخطوات في الخلفية بنجاح.");
        } else {
           console.log("مهمة التحقق من الخطوات مسجلة بالفعل.");
        }
        
      } catch (error) {
        console.error("حدث خطأ أثناء إعداد الإشعارات أو المهام الخلفية:", error);
      }
    };
  ...
  return (
    <Tab.Navigator
      tabBar={(props) => {
        ...
        if (currentTabRoute.name === 'DiaryStack') {
          const routeName = getFocusedRouteNameFromRoute(currentTabRoute) ?? 'DiaryHome';
          const screensToHideTabBar = ['Weight', 'Water', 'WorkoutLog', 'Steps', 'FoodLogDetail'];
    ...
    >
      <Tab.Screen name="DiaryStack" options={{ tabBarLabel: t('diaryTab'), tabBarIconName: 'journal-outline' }} >
          {props => <DiaryStackNavigator {...props} theme={theme} t={t} isRTL={isRTL} language={language} setHasProgress={setHasProgress} />}
      </Tab.Screen>
      <Tab.Screen name="ReportsStack" options={{ tabBarLabel: t('reportsTab'), tabBarIconName: 'stats-chart-outline' }} >
          {props => <ReportsStackNavigator {...props} theme={theme} />}
      </Tab.Screen>
...
const styles = { 
    ...
    searchContainer: (theme, isRTL) => ({ flexDirection: isRTL ? 'row-reverse' : 'row', padding: 15, backgroundColor: theme.card }),
    ...
    searchButton: (theme) => ({ width: 50, height: 50, backgroundColor: theme.primary, borderRadius: 10, justifyContent: 'center', alignItems: 'center', marginLeft: 10 }),
    resultItem: (theme, isRTL) => ({ flexDirection: isRTL ? 'row-reverse' : 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: theme.card, padding: 20, borderBottomWidth: 1, borderBottomColor: theme.background }),
    ...
    smallCardTitle: (theme) => ({ fontSize: 16, fontWeight: '600', color: theme.textPrimary, marginLeft: 8, marginRight: 8 }),
    ...
    // Food Log Item (in meals)
    foodLogItemContainer: (theme, isRTL) => ({ flexDirection: isRTL ? 'row-reverse' : 'row', alignItems: 'center', paddingVertical: 12, borderBottomWidth: 1, borderBottomColor: theme.background, }),
